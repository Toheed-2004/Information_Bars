...
from bitpredict.data.time_bars.metatrader5.fetcher import MetatraderFetcher
...
def run(symbol: str, exchange: str, run_mode: str, timeframes: dict) -> None:
    """
    Process a single symbol using the refactored Base with KrakenFetcher.
    
    Args:
        symbol: Trading symbol (e.g., "PF_XBTUSDT")
    ...
    data_handler = MetatraderFetcher(
        exchange=exchange,
        symbol=symbol
    )
...
def main():
    """
    Main entry point for the Metatrader5 data downloader.
    ...
    setup_logging("data.ochlv.metatrader5")
