"""
Binance Data Fetcher Main Entry Point - Database-Driven Version.

This module serves as the entry point for the binance cryptocurrency data
...
from fetcher import BinanceStreamManager
...
setup_logging("data.tick.binance")
logger = get_logger(__name__)


def main():
    """
    Main entry point for the binance data fetcher system (database-driven).
    ...
    4. Creates binanceStreamManager instance(s)
    ...
    Raises:
        RuntimeError: If no enabled binance configurations found in database.
    ...
    try:
        ...
        # Get all enabled configurations for binance
        binance_configs = get_tick_meta(exchange='binance')
        ...
        systems: List[BinanceStreamManager] = []
        
        for db_config in binance_configs:
            # Build config dict from database record
            config = build_config_from_db(db_config)
            
            logger.info(
                "Creating BinanceStreamManager for %s market...",
                db_config['market_type']
            )
            
            # Create stream manager
            system = BinanceStreamManager(config)
