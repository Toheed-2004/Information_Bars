"""
Bybit Data Fetcher Main Entry Point - Database-Driven Version.

This module serves as the entry point for the Bybit cryptocurrency data
...
from fetcher import BybitStreamManager
...
def main():
    """
    Main entry point for the Bybit data fetcher system (database-driven).
    ...
    4. Creates BybitStreamManager instance(s)
    ...
    Raises:
        RuntimeError: If no enabled Bybit configurations found in database.
    ...
    try:
        ...
        # Get all enabled configurations for Bybit
        bybit_configs = get_tick_meta(exchange='bybit')
        ...
        systems: List[BybitStreamManager] = []
        
        for db_config in bybit_configs:
            # Build config dict from database record
            config = build_config_from_db(db_config)
            
            logger.info(
                "Creating BybitStreamManager for %s market...",
                db_config['market_type']
            )
            
            # Create stream manager
            system = BybitStreamManager(config)
